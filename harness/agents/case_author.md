...
- fully green을 목표로 오라클을 약하게 만들지 말 것. expected 밖 후보와 forbidden source는
  향후 정밀화 계층을 평가하는 precision probe이므로 계속 설계하고 근거를
  적되, 양성 케이스의 forbidden 도달만으로 코어 recall 실패를 만들지 않는다.
