...
adapter -> Engine11 실행 -> FailureReport v2 -> suite summary -> gate까지
실제로 동작한다. Agent/ledger/cache 자동화는 다음 단계다.
...
| artifact hash, engine commit, expected hash, run config hash 기록 | `memory/store.py` 원장 구현(JSON/sqlite) |
...
## 다음 배선 순서
1. `memory/store.py`를 schema.json대로 구현(JSON로 시작 가능).
2. artifact_cache 4단 캐시(build/pcode/engine/verify)와 changed-only 실행을 붙인다.
3. local build/extract mode에서 `build.sh`, `extract_lowpcode.sh`, Ghidra headless를 adapter로 이동한다.
