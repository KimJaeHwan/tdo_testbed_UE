...
PR5 계층 캐시      : build/pcode/engine/verify 4단 + canonical 정규화
                     ...
                     engine/verify result는 pcode_hash + engine_commit + run_config_hash +
                     expected_hash가 동일할 때 cache hit로 skip한다. prepare step은
                     command + pinned env + input path hashes + outputs로 changed-only
                     cache hit를 판정한다. case-scoped 실행에서는 pcode_hash가 root JSON이
                     아니라 실제 분석 closure hash라 helper JSON 변경도 무효화된다)
...
PR7 gates 강화     : harness_defect triage(8범주+unknown) + evidence_required + known_frontier_not_counted
                     ...
                     review item으로 기록. --regression-baseline으로 이전 run id/output
                     dir/failure_report_v2.json을 지정하면 이전 PASS case가 현재 PASS를
                     유지하는지 I3_regression_zero를 판정)
PR8 capability/coverage_planner: missing/weakly/frontier/contradictory 관리
                     (2026-06-28 부분 완료: capability_map.json과 baseline_map.json을
                     run마다 갱신. FP/ERROR는 contradictory, missing-only fail은
                     human_confirmed=false frontier candidate로 기록. baseline.py로
                     named regression baseline pin을 관리하고 --regression-baseline에서
                     pin 이름을 직접 참조할 수 있다)
PR9 agent JSON 계약: schema validation + role 정규화 + proposed_cases approval queue
                     (2026-06-28 부분 완료: human_gate 기반 agent_tasks.json 생성.
